...
export default async function QuoteDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: quote } = await supabase.from('quotes').select('*, client_companies(*)').eq('id', id).single()
  ...
  const company = quote.client_companies as any
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" asChild>
            <Link href="/quotes">一覧に戻る</Link>
          </Button>
          <Button asChild>
            <Link href={`/quotes/${id}/edit`}>編集</Link>
          </Button>
        </div>
      </div>

      <div className="mb-6">
        <QuotePDF quote={quote} items={items || []} />
      </div>
