-- ON CONFLICT を使わないシンプル版マスターデータ投入スクリプト
...
DECLARE
  ...
  v_tax_rate_10 UUID;
  v_tax_rate_8 UUID;
  v_existing_tax_count INT;
  v_existing_account_count INT;
  v_existing_category_count INT;
BEGIN
  -- ユーザーIDを取得
  ...
  -- 既存データをカウント
  SELECT COUNT(*) INTO v_existing_tax_count
  FROM tax_rates WHERE tenant_id = v_tenant_id;

  SELECT COUNT(*) INTO v_existing_account_count
  FROM accounts WHERE tenant_id = v_tenant_id AND type = 'expense';

  SELECT COUNT(*) INTO v_existing_category_count
  FROM expense_categories WHERE tenant_id = v_tenant_id;

  RAISE NOTICE '========================================';
  RAISE NOTICE 'マスターデータ投入開始';
  RAISE NOTICE 'tenant_id: %', v_tenant_id;
  RAISE NOTICE 'user_id: %', v_user_id;
  RAISE NOTICE '既存データ: 税率=%件, 勘定科目=%件, カテゴリ=%件', v_existing_tax_count, v_existing_account_count, v_existing_category_count;
  RAISE NOTICE '========================================';

  -- ==========================================
  -- 1. 税率マスターを投入（既存データがない場合のみ）
  -- ==========================================
  IF v_existing_tax_count = 0 THEN
    ...
    RAISE NOTICE '  ✓ 税率マスター投入完了（4件）';
  ELSE
    RAISE NOTICE '1. 税率マスター: 既存データあり（%件）スキップ', v_existing_tax_count;
  END IF;

  -- ==========================================
  -- 2. 勘定科目マスターを投入（既存データがない場合のみ）
  -- ==========================================
  IF v_existing_account_count = 0 THEN
    ...
    RAISE NOTICE '  ✓ 勘定科目マスター投入完了（35件）';
  ELSE
    RAISE NOTICE '2. 勘定科目マスター: 既存データあり（%件）スキップ', v_existing_account_count;
  END IF;

  -- ==========================================
  -- 3. 経費カテゴリマスターを投入（既存データがない場合のみ）
  -- ==========================================
  IF v_existing_category_count = 0 THEN
    ...
    RAISE NOTICE '  ✓ 経費カテゴリマスター投入完了（8件）';
  ELSE
    RAISE NOTICE '3. 経費カテゴリマスター: 既存データあり（%件）スキップ', v_existing_category_count;
  END IF;

  RAISE NOTICE '';
  RAISE NOTICE '========================================';
  RAISE NOTICE '✓ マスターデータ投入が完了しました！';
  RAISE NOTICE '========================================';
END $$;
