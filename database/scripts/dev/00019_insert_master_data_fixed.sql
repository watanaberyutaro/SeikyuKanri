...
BEGIN
  ...
  INSERT INTO tax_rates (tenant_id, user_id, name, rate, category, is_compound, is_active, description)
  VALUES
    (v_tenant_id, v_user_id, '課税10%', 10.00, 'standard', false, true, '標準税率（消費税10%）'),
    (v_tenant_id, v_user_id, '課税8%（軽減税率）', 8.00, 'reduced', false, true, '軽減税率（食品等）'),
    (v_tenant_id, v_user_id, '非課税', 0.00, 'exempt', false, true, '非課税取引'),
    (v_tenant_id, v_user_id, '不課税', 0.00, 'non_taxable', false, true, '課税対象外'),
    (v_tenant_id, v_user_id, '免税', 0.00, 'zero_rated', false, true, '免税取引')
...
-- データサンプル
SELECT '【税率】' AS "確認";
SELECT name, rate FROM tax_rates WHERE tenant_id = 'b91ce4bb-a6f3-49a0-9bc5-f37082c9687c' ORDER BY rate DESC;
